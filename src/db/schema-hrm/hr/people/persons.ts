...
export const genderZodEnum = z.enum(genders);
...
export const maritalStatusZodEnum = z.enum(maritalStatuses);
...
export const personStatusZodEnum = z.enum(personStatuses);
...
export const personUpdateSchema = createUpdateSchema(persons, {
  ...
  maritalStatus: nullableOptional(maritalStatusZodEnum),
  nationality: nullableOptional(
    z.string().length(2).regex(/^[A-Z]{2}$/, "Must be ISO 3166-1 alpha-2 country code")
  ),
  primaryLanguage: nullableOptional(z.string().min(2).max(10)),
  photoUrl: nullableOptional(z.string().url().max(500)),
});
