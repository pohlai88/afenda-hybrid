 ...
 *
 * Note: shiftAssignmentId FK added via custom SQL (time ↔ operations import boundary).
 * Note: timesheetId FK added via custom SQL (time ↔ operations import boundary).
 */
...
export const attendanceTypeZodEnum = z.enum(attendanceTypes);
...
}).superRefine((data, ctx) => refineCheckOutAfterCheckIn(data, ctx));

export const attendanceLogUpdateSchema = createUpdateSchema(attendanceLogs, {
  shiftAssignmentId: nullableOptional(z.number().int()),
  timesheetId: nullableOptional(z.number().int()),
  checkInAt: timestamptzWireNullableOptionalSchema,
  checkOutAt: timestamptzWireNullableOptionalSchema,
