...
import { nonNegativeDecimalString } from "../_zodShared";
...
export const benefitApplicationDetails = benefitsSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade"),
    check(
      "chk_benefit_application_details_amount",
      sql`${t.electedAmount} >= 0`
    ),
  ]
);

export const BenefitApplicationDetailIdSchema = z.number().int().positive().brand<"BenefitApplicationDetailId">();
