...
import {
  ...
  dateStringSchema,
  nullableOptional,
  refineExpiryDateOnOrAfterAwardDate,
} from "../_zodShared";
...
export const awardStatusZodEnum = z.enum(awardStatuses);
...
/** Read model: trust DB + expiry `CHECK`; use insert/update for write rules. */
...
export const certificationAwardInsertSchema = createInsertSchema(certificationAwards, {
  awardDate: dateStringSchema,
  expiryDate: dateOptionalSchema,
  certificateNumber: certificateNumberSchema.optional(),
  certificateUrl: contentUrlSchema.optional(),
  status: awardStatusZodEnum.optional(),
}).superRefine(refineExpiryDateOnOrAfterAwardDate);
...
export const certificationAwardUpdateSchema = createUpdateSchema(certificationAwards, {
  certificateNumber: nullableOptional(certificateNumberSchema),
  certificateUrl: nullableOptional(contentUrlSchema),
  awardDate: dateStringSchema.optional(),
  expiryDate: dateNullableOptionalSchema,
  status: awardStatusZodEnum.optional(),
}).superRefine(refineExpiryDateOnOrAfterAwardDate);
