...
1. **Relations** — Only `_relations.ts` defines `defineRelations()`. It is merged in `src/db/db.ts` with other domains for `db.query.*`. Contract-style coverage: `src/db/__tests__/payroll-rqb-nested-relations.test.ts`.
...
```bash
pnpm vitest run --config vitest.db.config.ts src/db/__tests__/payroll-
pnpm vitest run --config vitest.db.config.ts src/db/__tests__/payment-records-zod.test.ts
pnpm vitest run src/db/__tests__/payroll-zod-shared.test.ts
```
