 ...
 */
export const gratuitySettlementStatuses = ["DRAFT", "CALCULATED", "APPROVED", "PAID", "CANCELLED"] as const;

export const gratuitySettlementStatusEnum = payrollSchema.enum("gratuity_settlement_status", [...gratuitySettlementStatuses]);
...
const yearsOfServiceSchema = z.string().regex(/^\d{1,3}(\.\d{1,2})?$/, "must be valid numeric(5,2) >= 0");
...
export const gratuitySettlements = payrollSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade"),
    check(
      "chk_gratuity_settlements_years",
      sql`${t.totalYears} >= 0`
    ),
    check(
      "chk_gratuity_settlements_amount",
      sql`${t.amount} >= 0`
    ),
  ]
);

export const GratuitySettlementIdSchema = z.number().int().positive().brand<"GratuitySettlementId">();
