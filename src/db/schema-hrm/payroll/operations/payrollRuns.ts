...
function refineTotalsNetEqualsGrossMinusDeductions(
  ...
  if (!isValidMoney14_2NonNegative(g) || !isValidMoney14_2NonNegative(d) || !isValidMoney14_2NonNegative(n)) return;
...
function refineEmployeeCountVsTotalsUpdate(data: Record<string, unknown>, ctx: z.RefinementCtx): void {
  if (!Object.prototype.hasOwnProperty.call(data, "employeeCount") || (data.employeeCount as number) !== 0) {
    return;
  }
...
function refinePayrollRunInsertWorkflow(data: PayrollRunInsertWorkflow, ctx: z.RefinementCtx): void {
  ...
  const preApproval = status === "DRAFT" || status === "CALCULATING" || status === "PENDING_APPROVAL";
  if (preApproval) {
    if (hasApprovedBy || hasApprovedAt || hasProcessedBy || hasProcessedAt) {
      ctx.addIssue({
        code: "custom",
        message: "Approval and processing fields must be unset for DRAFT, CALCULATING, and PENDING_APPROVAL",
        path: ["status"],
      });
    }
    return;
  ...
  if (status === "APPROVED") {
    ...
    if (hasProcessedBy || hasProcessedAt) {
      ctx.addIssue({
        code: "custom",
        message: "processedBy and processedAt must be unset until status is PROCESSING or COMPLETED",
        path: ["status"],
      });
    }
    return;
  ...
  if (status === "PROCESSING" || status === "COMPLETED") {
    if (!hasApprovedBy || !hasApprovedAt) {
      ctx.addIssue({
        code: "custom",
        message: "approvedBy and approvedAt are required (run must be approved before PROCESSING/COMPLETED)",
        path: ["approvedBy"],
      });
    }
    if (!hasProcessedBy || !hasProcessedAt) {
...
function refinePayrollRunUpdateWorkflow(data: Record<string, unknown>, ctx: z.RefinementCtx): void {
  ...
  if (preApproval) {
    if (
      ...
      ctx.addIssue({
        code: "custom",
        message: "Cannot set approval or processing fields when moving to DRAFT, CALCULATING, or PENDING_APPROVAL",
        path: ["status"],
      });
    }
    return;
  ...
  if (status === "PROCESSING" || status === "COMPLETED") {
    if (!approvedByTouched || approvedBy == null || !Number.isFinite(approvedBy) || approvedBy <= 0) {
      ctx.addIssue({
        code: "custom",
        message: "When setting status to PROCESSING or COMPLETED, include approvedBy in the same patch",
        path: ["approvedBy"],
      });
    }
    if (!approvedAtTouched || approvedAt == null) {
      ctx.addIssue({
        code: "custom",
        message: "When setting status to PROCESSING or COMPLETED, include approvedAt in the same patch",
        path: ["approvedAt"],
      });
    }
    if (!processedByTouched || processedBy == null || !Number.isFinite(processedBy) || processedBy <= 0) {
      ctx.addIssue({
        code: "custom",
        message: "When setting status to PROCESSING or COMPLETED, include processedBy in the same patch",
        path: ["processedBy"],
      });
    }
    if (!processedAtTouched || processedAt == null) {
      ctx.addIssue({
        code: "custom",
        message: "When setting status to PROCESSING or COMPLETED, include processedAt in the same patch",
        path: ["processedAt"],
      });
    }
  }
...
export const payrollRunInsertSchema = payrollRunInsertSchemaBase.superRefine((data, ctx) => {
  refineTotalsNetEqualsGrossMinusDeductions(data.totalGross, data.totalDeductions, data.totalNet, ctx);
  refineEmployeeCountVsTotalsInsert(data.employeeCount, data.totalGross, data.totalDeductions, data.totalNet, ctx);
