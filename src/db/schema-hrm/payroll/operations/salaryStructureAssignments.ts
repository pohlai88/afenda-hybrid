...
export const salaryStructureAssignments = payrollSchema.table(
  ...
  (t) => [
    ...
    index("idx_salary_structure_assignments_effective").on(t.tenantId, t.employeeId, t.effectiveFrom),
    foreignKey({
      columns: [t.tenantId],
    ...
    check(
      "chk_salary_structure_assignments_base_salary",
      sql`${t.baseSalary} > 0`
    ),
  ]
);

export const SalaryStructureAssignmentIdSchema = z.number().int().positive().brand<"SalaryStructureAssignmentId">();
...
export const salaryStructureAssignmentInsertSchema = createInsertSchema(salaryStructureAssignments, {
...
export const salaryStructureAssignmentUpdateSchema = createUpdateSchema(salaryStructureAssignments, {
