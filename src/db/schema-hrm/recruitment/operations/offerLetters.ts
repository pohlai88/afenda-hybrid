 ...
 * that equality. Use `createOfferLetter` in `src/db/_services/recruitment/offerLettersService.ts` for application-layer creates.
 *
...
export const offerLetterInsertSchema = createInsertSchema(offerLetters, {
  ...
  signingBonus: z
    .string()
    .optional()
    .refine((val) => val === undefined || val === "" || parseFloat(val) >= 0, "Signing bonus must be non-negative"),
...
export const offerLetterUpdateSchema = createUpdateSchema(offerLetters, {
  ...
  offerCode: z.string().min(2).max(50).regex(/^[A-Z0-9_-]+$/i, "Only alphanumeric, underscore, and hyphen allowed").optional(),
  baseSalary: z.string().refine((val) => parseFloat(val) > 0, "Base salary must be positive").optional(),
  signingBonus: z
    .string()
    .optional()
    .refine((val) => val === undefined || val === "" || parseFloat(val) >= 0, "Signing bonus must be non-negative"),
