...
export const staffingPlanDetails = recruitmentSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade"),
    check(
      "chk_staffing_plan_details_current_count",
      sql`${t.currentCount} >= 0`
    ),
    check(
      "chk_staffing_plan_details_planned_count",
      sql`${t.plannedCount} >= 0`
    ),
    check(
...
export const StaffingPlanDetailIdSchema = z.number().int().positive().brand<"StaffingPlanDetailId">();
...
export const staffingPlanDetailInsertSchema = createInsertSchema(staffingPlanDetails, {
  ...
  estimatedCostPerPosition: z.string().refine(isValidMoney12_2, "must be valid numeric(12,2) >= 0").optional().nullable(),
