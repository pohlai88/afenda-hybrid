 ...
 * `src/db/_services/recruitment/candidateSalaryBackfillIssuesService.ts`.
 *
...
export const candidateSalaryBackfillIssues = recruitmentSchema.table(
  ...
  (t) => [
    uniqueIndex("uq_candidate_salary_backfill_issues_tenant_candidate").on(t.tenantId, t.candidateId),
...
export const CandidateSalaryBackfillIssueIdSchema = z.number().int().brand<"CandidateSalaryBackfillIssueId">();
export type CandidateSalaryBackfillIssueId = z.infer<typeof CandidateSalaryBackfillIssueIdSchema>;

export const candidateSalaryBackfillIssueSelectSchema = createSelectSchema(candidateSalaryBackfillIssues);
...
function refineNormalizedDigits(row: Record<string, unknown>, ctx: z.RefinementCtx) {
  ...
  if (!/^\d+(\.\d+)?$/.test(v)) {
    ctx.addIssue({
      ...
      message: "normalizedDigits must be digits with an optional decimal fraction (e.g. 50000 or 50000.50)",
      path: ["normalizedDigits"],
    });
  }
}

export const candidateSalaryBackfillIssueInsertSchema = createInsertSchema(candidateSalaryBackfillIssues, {
...
export const candidateSalaryBackfillIssueUpdateSchema = createUpdateSchema(candidateSalaryBackfillIssues, {
