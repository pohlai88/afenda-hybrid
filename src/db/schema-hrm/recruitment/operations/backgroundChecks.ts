...
export const backgroundChecks = recruitmentSchema.table(
  ...
  (t) => [
    ...
    index("idx_background_checks_candidate_requested_date").on(t.tenantId, t.candidateId, t.requestedDate),
...
}).superRefine((row, ctx) => refineBackgroundCheckResultVsStatus(row as Record<string, unknown>, ctx, "insert"));
