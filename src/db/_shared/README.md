...
| **Schema packages** (`schema-hrm/*`, `schema-platform/*`) | Typically `from "../../_shared"` or **`@db/_shared`** when crossing many levels — both resolve to this folder; prefer **`@db/_shared`** for consistency with other `src/db` infra. |
...
Behavior is mostly enforced by CI gates and schema tests. If you add Zod-wire unit tests, place them under [`src/db/__tests__/`](../__tests__/) and import via **`@db/_shared`** where appropriate.
