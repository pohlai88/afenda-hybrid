 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/roles-zod.test.ts
 */
import { describe, expect, it } from "vitest";
import {
  roleInsertSchema,
  roleUpdateSchema,
} from "../schema-platform/security/roles";
...
describe("role Zod schemas", () => {
  describe("roleInsertSchema — roleCode", () => {
    it("rejects roleCode with spaces", () => {
      expectZodIssueAtPath(roleInsertSchema, { ...validInsertBase, roleCode: "BAD CODE" }, "roleCode");
    });

    it("rejects roleCode with disallowed special characters", () => {
      expectZodIssueAtPath(roleInsertSchema, { ...validInsertBase, roleCode: "BAD@ROLE" }, "roleCode");
    });
