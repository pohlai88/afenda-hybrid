 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/payroll-entries-zod.test.ts
 *
 * Duplicate lines per run/employee/component are not constrained at DB level — business rule in service if needed.
 */
import { describe, expect, it } from "vitest";
import { payrollEntryInsertSchema, payrollEntryUpdateSchema } from "../schema-hrm/payroll/operations/payrollEntries";
...
describe("payroll entry Zod schemas", () => {
  describe("payrollEntryInsertSchema", () => {
    it("rejects description shorter than 2 characters after trim", () => {
      expectZodIssueAtPath(payrollEntryInsertSchema, { ...baseInsert, description: "x" }, "description");
    });
