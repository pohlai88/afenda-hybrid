 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/final-settlements-zod.test.ts
 */
...
describe("final settlement Zod schemas", () => {
  ...
  describe("finalSettlementUpdateSchema", () => {
    ...
    it("rejects partial monetary totals patch", () => {
      expectZodIssueAtPath(finalSettlementUpdateSchema, { totalEarnings: "500.00" }, "netSettlement");
    });
    ...
    it("rejects paidAt without status PAID in the same patch", () => {
      expectZodIssueAtPath(finalSettlementUpdateSchema, { paidAt: new Date("2026-04-01") }, "paidAt");
    });
...
describe("final settlements DB constraints", () => {
  ...
  it("rejects a second non-cancelled settlement for the same employee", async () => {
    ...
    await db.insert(finalSettlements).values(
