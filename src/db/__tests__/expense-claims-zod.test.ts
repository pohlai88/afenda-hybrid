 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/expense-claims-zod.test.ts
 */
...
describe("expense claim Zod schemas", () => {
  describe("expenseClaimInsertSchema", () => {
    ...
    it("rejects amount above numeric(10,2) magnitude", () => {
      expectZodIssueAtPath(expenseClaimInsertSchema, { ...baseInsert, amount: "100000000.00" }, "amount");
    });
