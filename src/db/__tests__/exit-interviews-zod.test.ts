 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/exit-interviews-zod.test.ts
 */
...
describe("exit interview Zod schemas", () => {
  describe("exitInterviewInsertSchema", () => {
    ...
    it("rejects non-positive tenantId", () => {
      expect(exitInterviewInsertSchema.safeParse({ ...validInsertBase, tenantId: 0 }).success).toBe(false);
    });
  });
