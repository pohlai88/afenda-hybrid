 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/offer-letters-zod.test.ts
 */
import { describe, expect, it } from "vitest";
import { offerLetterInsertSchema, offerLetterUpdateSchema } from "../schema-hrm/recruitment/operations/offerLetters";
...
describe("offer letter Zod schemas", () => {
  describe("offerLetterInsertSchema", () => {
    ...
    it("rejects non-positive tenantId", () => {
      expect(offerLetterInsertSchema.safeParse({ ...validInsertBase, tenantId: 0 }).success).toBe(false);
    });

    it("rejects invalid status on insert", () => {
      expectInvalidEnumField(offerLetterInsertSchema, validInsertBase, "status", "NOT_A_REAL_STATUS");
    });
  });
