 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/benefits-optional-relations.test.ts
 */
...
describe("benefits optional RQB relations", () => {
  ...
  it("claimsRecord reviewer is absent when reviewedBy is null", async () => {
    const row = await db.query.claimsRecords.findFirst({
      where: { AND: [{ tenantId: { eq: tenantId } }, { claimRecordId: { eq: claimNoReviewerId } }] },
      with: { reviewer: true },
    });
    expect(row).toBeDefined();
  ...
  it("claimsRecord resolves reviewer employee when reviewedBy is set", async () => {
    const row = await db.query.claimsRecords.findFirst({
      where: { AND: [{ tenantId: { eq: tenantId } }, { claimRecordId: { eq: claimWithReviewerId } }] },
      with: { reviewer: true },
    });
    expect(row).toBeDefined();
