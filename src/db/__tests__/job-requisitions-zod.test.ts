 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/job-requisitions-zod.test.ts
 */
...
describe("job requisition Zod schemas", () => {
  describe("jobRequisitionInsertSchema", () => {
    ...
    it("rejects headcount below 1", () => {
      expect(jobRequisitionInsertSchema.safeParse({ ...validInsertBase, headcount: 0 }).success).toBe(false);
    });
  });
