 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/learning-ld-enrollments-zod.test.ts
 */
...
import { trainingEnrollmentInsertSchema, trainingEnrollmentUpdateSchema } from "../schema-hrm/learning/operations/trainingEnrollments";
...
describe("learning L&D Zod — completion vs status", () => {
  ...
  it("learningPathCourseProgressInsertSchema rejects COMPLETED without completionDate", () => {
    expectZodIssueAtPath(learningPathCourseProgressInsertSchema, { ...basePathProgress, status: "COMPLETED" }, "completionDate");
  });

  it("learningPathCourseProgressUpdateSchema applies same completion rule", () => {
    expectZodIssueAtPath(learningPathCourseProgressUpdateSchema, { status: "COMPLETED" }, "completionDate");
  });
});
