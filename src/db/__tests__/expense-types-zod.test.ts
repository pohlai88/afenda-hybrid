 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/expense-types-zod.test.ts
 *
 * Duplicate `expenseCode` per tenant is enforced by DB unique index, not Zod.
 */
import { describe, expect, it } from "vitest";
import { expenseTypeInsertSchema, expenseTypeUpdateSchema } from "../schema-hrm/payroll/fundamentals/expenseTypes";
