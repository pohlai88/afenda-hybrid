 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/disciplinary-action-zod.test.ts
 */
  ...
  describe("disciplinaryActionInsertSchema", () => {
    ...
    it("rejects expiryDate before issueDate when set", () => {
      ...
      if (!r.success) {
        ...
        expect(
          r.error.issues.some((i) => String(i.message).includes("Expiry date")),
        ).toBe(true);
      }
    });
