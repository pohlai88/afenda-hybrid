 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/candidate-salary-backfill-issues-fk.test.ts
 */
...
describe.skipIf(!process.env.DATABASE_URL)("candidate_salary_backfill_issues tenant FK", () => {
  ...
  afterAll(async () => {
    await db.delete(candidateSalaryBackfillIssues).where(eq(candidateSalaryBackfillIssues.candidateId, candidateId));
  ...
  it("rejects insert when tenantId does not exist in core.tenants", async () => {
    ...
    ).rejects.toSatisfy(matchesPgError(/23503|foreign key|fk_candidate_salary_backfill_issues_tenant/i));
  });
