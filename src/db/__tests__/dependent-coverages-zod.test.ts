 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/dependent-coverages-zod.test.ts
 */
...
describe("dependent coverage Zod schemas", () => {
  ...
  describe("dependentCoverageUpdateSchema", () => {
    ...
    it("allows clearing terminationReason with null when not setting TERMINATED", () => {
      expect(dependentCoverageUpdateSchema.safeParse({ terminationReason: null }).success).toBe(true);
    });
  });
});
