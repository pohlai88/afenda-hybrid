 ...
 * Run: pnpm vitest run --config vitest.db.config.ts src/db/__tests__/earnings-types-zod.test.ts
 *
 * Duplicate `earningsCode` per tenant is enforced by DB unique index, not Zod.
 */
import { describe, expect, it } from "vitest";
import { earningsTypeInsertSchema, earningsTypeUpdateSchema } from "../schema-hrm/payroll/fundamentals/earningsTypes";
