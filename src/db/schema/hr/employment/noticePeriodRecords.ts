...
export const noticeInitiatorZodEnum = createSelectSchema(noticeInitiatorEnum);
...
export const noticeStatusZodEnum = createSelectSchema(noticeStatusEnum);
...
export const noticePeriodRecords = hrSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade"),
    check(
      "chk_notice_period_records_expected_date",
      sql`${t.expectedLastDay} >= ${t.noticeDate}`
    ),
    check(
    ...
    check(
      "chk_notice_period_records_notice_days",
      sql`${t.requiredNoticeDays} >= 0`
    ),
  ]
);
