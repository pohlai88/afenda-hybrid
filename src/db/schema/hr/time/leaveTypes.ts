import { integer, text, smallint, boolean, index, uniqueIndex, foreignKey, check } from "drizzle-orm/pg-core";
...
export const leaveTypeStatusZodEnum = createSelectSchema(leaveTypeStatusEnum);
...
export const leaveTypeInsertSchema = createInsertSchema(leaveTypes, {
  leaveTypeCode: z.string().min(2).max(50).regex(/^[A-Z0-9_-]+$/i, "Only alphanumeric, underscore, and hyphen allowed"),
