import { integer, varchar, date, smallint, index, uniqueIndex, foreignKey, check } from "drizzle-orm/pg-core";
 ...
 * Preflight: `docs/preflight-succession-plans-lifecycle.sql`.
 *
 * Optional next step — require non-empty `developmentPlan` for `ACTIVE` / `UNDER_REVIEW`: see
 * `docs/succession-plans-optional-development-plan-check.md` (not enforced in DB until product approves).
 * Nightly staging gap count (optional): `pnpm report:succession-plans-development-gap` — `docs/CI_GATES.md`.
 * Metrics JSON line: `SUCCESSION_DEV_PLAN_GAP_JSON_LINE=1`; job summary: `GITHUB_STEP_SUMMARY` — `docs/succession-plans-optional-development-plan-check.md`.
 */
