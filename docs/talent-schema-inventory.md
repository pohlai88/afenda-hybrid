...
Full entries: [`src/db/schema/audit/CUSTOM_SQL_REGISTRY.json`](../src/db/schema/audit/CUSTOM_SQL_REGISTRY.json).

## Adding or renaming a talent table

Do these in one change set (or follow-up PR) so docs, gates, and DB stay aligned:

1. **Drizzle** — table module under [`src/db/schema/talent`](../src/db/schema/talent), barrel exports (`index.ts` / `_schema.ts` if applicable), and [`_relations.ts`](../src/db/schema/talent/_relations.ts).
2. **Migrations** — generated/applied SQL; register custom SQL or triggers in [`CUSTOM_SQL_REGISTRY.json`](../src/db/schema/audit/CUSTOM_SQL_REGISTRY.json) when needed.
