...
| Joining tasks              | `recruitment.onboarding_checklists`  | Per-employee checklist rows (`taskCategory`, `status`, assignee, dates)                                                                                                                                                                |
| Leaving / exit tasks       | `recruitment.offboarding_checklists` | Per-employee exit checklist (`offboarding_task_category`, reuses `onboarding_task_status`); termination / pay / benefits still on core tables — see [hr-lifecycle-onboarding-offboarding.md](./hr-lifecycle-onboarding-offboarding.md) |
| Exit interview (departure) | `recruitment.exit_interviews`        | Schedule/conduct, format, themes, summary, `wouldRehire`; **`linkedOffboardingChecklistId` required** → active `offboarding_checklists` row with `EXIT_INTERVIEW` (enforced by CSQL-015 triggers). Not ATS `recruitment.interviews`.   |
