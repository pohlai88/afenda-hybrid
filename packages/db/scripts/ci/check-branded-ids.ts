  ...
  if (hasBrandedSchema) {
    ...
    if (pkColumn.type === "bigint") {
      ...
      const modeMatch = content.match(
        new RegExp(`${pkColumn.name}:\\s*bigint\\(\\{[^}]*mode:\\s*["']([^"']+)["']`)
      );
      expectedZodType = modeMatch && modeMatch[1] === "bigint" ? "bigint" : "number";
    } else if (
      pkColumn.type === "text" ||
      pkColumn.type === "varchar" ||
      pkColumn.type === "uuid"
    ) {
    ...
    const brandedSchemaMatch = content.match(
      new RegExp(`${expectedSchemaName}\\s*=\\s*z\\.(\\w+)\\(`)
    );
...
function checkForeignKeyBrandedIds(
  table: TableInfo,
  _schema: SchemaInfo,
  _allSchemas: SchemaInfo[]
): void {
  const content = fs.readFileSync(table.file, "utf-8");
