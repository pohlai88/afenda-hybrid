  ...
  entries
    ...
    .forEach((entry) => {
      ...
      if (!TIER_1_2_SCHEMAS.includes(schemaName)) {
        const hasFundamentals = fs.existsSync(path.join(schemaPath, "fundamentals"));
        const hasOperations = fs.existsSync(path.join(schemaPath, "operations"));
        ...
        if (!hasFundamentals && !hasOperations) {
          ...
          if (tableCount > 3) {
            errors.push({
              file: `src/schema-platform/${schemaName}`,
              line: 1,
              column: 1,
...
function checkSharedMixins(): void {
  const sharedPath = path.join(SRC_DIR, "_shared");
  ...
  if (!fs.existsSync(sharedPath)) {
    errors.push({
      ...
      severity: "error",
      suggestion: "Create src/_shared/ with timestamps.ts, auditColumns.ts, and index.ts",
    });
    return;
  }
...
function checkMainIndex(): void {
  ...
  if (!fs.existsSync(mainIndexPath)) {
    errors.push({
      file: "src/schema-platform/index.ts",
      line: 1,
      column: 1,
      rule: "main-barrel",
    ...
    .forEach((entry) => {
      ...
      if (nestedDirs.length > 0) {
        nestedDirs.forEach((nested) => {
          checkSchemaDirectory(path.join(schemaPath, nested.name), `${entry.name}/${nested.name}`);
        });
      }
