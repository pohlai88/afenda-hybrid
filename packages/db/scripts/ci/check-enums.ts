...
const DB_DIR = path.join(process.cwd(), "src");
...
function extractEnums(filePath: string): void {
  ...
  const enumRegex =
    /export\s+const\s+(\w+)\s*=\s*(?:schema\.)?pgEnum\(\s*["'](\w+)["']\s*,\s*\[([^\]]+)\]/g;
  ...
  while ((match = enumRegex.exec(content)) !== null) {
    ...
    const hasTypeExport =
      content.includes(`type ${varName.replace(/Enum$/, "")}`) ||
  ...
  for (let i = 0; i < enumNames.length; i++) {
    for (let j = i + 1; j < enumNames.length; j++) {
      ...
      if (name1 === name2 || name1.includes(name2) || name2.includes(name1)) {
        ...
        if (
          overlap.length > 0 &&
          overlap.length >= Math.min(info1.values.length, info2.values.length) / 2
        ) {
          issues.push({
            file: info1.file,
            line: info1.line,
            ...
            severity: "warning",
            suggestion:
              "Consider consolidating into a single enum if they represent the same concept",
          });
        }
      }
    }
...
function main(): void {
  ...
  if (issues.length === 0) {
    ...
    for (const [name, info] of enums) {
      console.log(
        `  - ${name}: ${info.values.length} values, used in ${info.usedInTables.length} table(s)`
      );
    }
