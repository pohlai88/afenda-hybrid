...
const EXEMPT_TABLES = ["tenants", "regions", "locations", "audit_trail", "traces"];
...
function checkTenantIsolation(filePath: string): void {
  ...
  if (!hasTenantFK) {
    ...
    issues.push({
      ...
      suggestion:
        "Add foreignKey({ columns: [t.tenantId], foreignColumns: [tenants.tenantId], name: 'fk_..._tenant' })",
    });
  }

  // Check unique constraints include tenantId
  const uniqueMatches = content.matchAll(
    /unique(?:Index)?\(["']([^"']+)["']\)[^;]*\.on\(([^)]+)\)/g
  );
  ...
  for (const match of inlineUniqueMatches) {
    ...
    if (colName && colName !== "tenantId") {
      ...
      issues.push({
        ...
        suggestion:
          "Replace with composite uniqueIndex including tenantId:\n   uniqueIndex('uq_table_column').on(t.tenantId, t.column)",
      });
    }
  }
