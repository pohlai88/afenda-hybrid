...
const DATABASE_URL =
  process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/afenda_sql_check";
...
function extractCustomSqlBlocks(
  sqlContent: string
): Array<{ id: string; sql: string; line: number }> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      inCustomBlock &&
      (line.includes("--> statement-breakpoint") || line.startsWith("-- CUSTOM:"))
    ) {
...
function validateSqlSyntax(
  ...
  try {
    ...
    console.log(
      `⚠️ Could not connect to database for syntax check: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
