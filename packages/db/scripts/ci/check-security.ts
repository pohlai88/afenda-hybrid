...
/** Reviewed migrations that intentionally create NOLOGIN application roles (RLS). */
const CREATE_ROLE_MIGRATION_ALLOWLIST = new Set([
  "src/migrations/20260321130000_rls_app_roles/migration.sql",
]);

/** Strip SQL line comments and block comments so comment text does not trip migration scanners. */
function stripSqlComments(sql: string): string {
  const noLine = sql.replace(/--[^\n]*/g, "\n");
...
const DANGEROUS_MIGRATION_PATTERNS = [
  ...
  { pattern: /\bCREATE\s+DATABASE\b/i, severity: "high" as const, name: "create-database" },
  { pattern: /\bCREATE\s+ROLE\b/i, severity: "high" as const, name: "create-role" },
  ...
  {
    pattern: /pg_execute_server_program/i,
    severity: "critical" as const,
    name: "server-program-execution",
  },
];
    ...
    if (pattern.test(content)) {
      ...
      if (match) {
        ...
        const context = content.substring(
          Math.max(0, content.indexOf(match[0]) - 30),
...
function checkMigrationSecurity(): void {
  ...
  for (const migration of migrations) {
    ...
    const relativePath = `src/migrations/${migration}/migration.sql`;
    const contentNoComments = stripSqlComments(content);

    for (const { pattern, severity, name } of DANGEROUS_MIGRATION_PATTERNS) {
      if (!pattern.test(contentNoComments)) continue;
      if (name === "create-role" && CREATE_ROLE_MIGRATION_ALLOWLIST.has(relativePath)) continue;
      ...
      issues.push({
        ...
        severity,
        suggestion:
          "Review this operation carefully. Ensure it's necessary and has proper safeguards.",
      });
    }
