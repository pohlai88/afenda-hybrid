...
function extractColumnsWithLines(
  content: string
): { name: string; line: string; lineNum: number; colNum: number }[] {
  const columns: { name: string; line: string; lineNum: number; colNum: number }[] = [];
  const lines = content.split("\n");

  const columnRegex =
    /^\s*(\w+):\s*(integer|text|varchar|boolean|timestamp|date|jsonb|json|bigint|smallint|real|doublePrecision|numeric|uuid)\s*\([^)]*\)/;
...
function checkFile(filePath: string, exceptions: Exception[]): void {
  ...
  for (const col of columns) {
    ...
    const typeMatch = col.line.match(
      /(integer|text|varchar|boolean|timestamp|date|jsonb|json|bigint|smallint|real|doublePrecision|numeric|uuid)/
    );
    if (typeMatch) {
