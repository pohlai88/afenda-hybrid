...
const REGISTRY_PATH = path.join(
  process.cwd(),
  "src/schema-platform/audit/CUSTOM_SQL_REGISTRY.json"
);
const SCHEMA_PATH = path.join(
  process.cwd(),
  "src/schema-platform/audit/CUSTOM_SQL_REGISTRY.schema.json"
);
...
function validateJsonSyntax(): Registry | null {
  if (!fs.existsSync(REGISTRY_PATH)) {
    issues.push({
      ...
      severity: "error",
      suggestion: "Create the registry file at src/schema-platform/audit/CUSTOM_SQL_REGISTRY.json",
    });
    return null;
  }
    ...
    if (!entry.migration) {
      ...
      if (!migrationPattern.test(entry.migration)) {
        issues.push({
          ...
          severity: "error",
          suggestion:
            "Use format: YYYYMMDDHHMMSS_descriptive_name (e.g., 20260319144405_gifted_ultragirl)",
        });
      }
    }
...
function checkForOrphanedMarkers(): void {
  ...
  const registryIds = new Set(
    Object.keys(JSON.parse(fs.readFileSync(REGISTRY_PATH, "utf-8")).entries || {})
  );
