...
/** Concatenated migration.sql contents — RLS is often applied here instead of in Drizzle TS. */
function loadAllMigrationSql(): string {
  ...
  for (const entry of fs.readdirSync(migrationsDir, { withFileTypes: true })) {
    ...
    if (fs.existsSync(sqlPath)) {
      blob += fs.readFileSync(sqlPath, "utf-8");
      blob += "\n";
    }
  }
  return blob;
...
function tableHasRlsEnabledInMigrations(
  pgSchema: string,
  tableName: string,
  migrationBlob: string
): boolean {
  if (!migrationBlob) return false;
  const re = new RegExp(
    `ALTER\\s+TABLE\\s+"${escapeRegExp(pgSchema)}"\\."${escapeRegExp(tableName)}"\\s+ENABLE\\s+ROW\\s+LEVEL\\s+SECURITY`,
    "i"
  );
...
function checkRlsEnabled(table: TableInfo, schema: SchemaInfo, migrationBlob: string): void {
  ...
  const hasRlsInSchema =
    content.includes("withRLS") || content.includes("pgPolicy") || content.includes("enableRLS");
  const hasRlsInMigrations = tableHasRlsEnabledInMigrations(schema.name, table.name, migrationBlob);

  if (!hasRlsInSchema && !hasRlsInMigrations) {
    issues.push({
      ...
      severity: "error",
      suggestion:
        "Add RLS policy via custom SQL migration: ALTER TABLE ... ENABLE ROW LEVEL SECURITY; CREATE POLICY tenant_isolation ...",
    });
  }
}
...
function checkRlsPolicies(table: TableInfo, _schema: SchemaInfo): void {
  ...
  for (const policy of policies) {
    ...
    const validPattern = new RegExp(
      `^(${table.name}|tenant)_\\w+_(policy|select|insert|update|delete)$`,
      "i"
    );
  ...
  for (const match of policyBlocks) {
    ...
    if (!policyBlock.includes("tenant") && !policyBlock.includes("tenantId")) {
      issues.push({
        ...
        severity: "warning",
        suggestion:
          "Add tenant_id check: using(tenant_id = current_setting('afenda.tenant_id')::integer)",
      });
    }
  }
...
function checkMigrationRls(): void {
  ...
  for (const migration of migrations) {
    ...
    if (
      ...
      issues.push({
        file: `src/migrations/${migration}/migration.sql`,
...
function main(): void {
  ...
  const migrationBlob = loadAllMigrationSql();

  // Run checks
  for (const schema of schemas) {
    for (const table of schema.tables) {
      checkRlsEnabled(table, schema, migrationBlob);
