...
/** Platform (core, security, audit) + HRM domains (hr, payroll, …) — matches `db.ts` relation merge. */
const SCHEMA_ROOTS = [
  path.join(process.cwd(), "src/schema-platform"),
  path.join(process.cwd(), "src/schema-hrm"),
];

function relationsRelativePath(schema: SchemaInfo): string {
  return path.relative(process.cwd(), path.join(schema.path, "_relations.ts")).replace(/\\/g, "/");
}

function loadAllSchemas(): SchemaInfo[] {
  return SCHEMA_ROOTS.flatMap((root) => analyzeSchema(root));
}
...
function checkForeignKeyRelations(table: TableInfo, schema: SchemaInfo): void {
  ...
  for (const fk of table.foreignKeys) {
    ...
    if (!hasRelation) {
      issues.push({
        file: relationsRelativePath(schema),
        line: 1,
        table: table.name,
    ...
    if (!manyRelations.includes(oneRel)) {
      ...
      if (!isExcepted(exceptions, oneRel, "missing-bidirectional-relation")) {
        issues.push({
          ...
          severity: "info",
          suggestion: `Consider adding inverse relation on ${oneRel} back to this domain (r.many.${schema.name}(...) or equivalent)`,
        });
      }
    }
  }
...
function main(): void {
  ...
  const platformRoot = path.join(process.cwd(), "src/schema-platform");
  ...
  for (const schema of schemas) {
    ...
    // Heuristic r.one/r.many pairing is noisy across HRM (many cross-domain r.one); keep it platform-only.
    if (schema.path.startsWith(platformRoot)) {
