...
function checkCompositeIndexPatterns(table: TableInfo, _schema: SchemaInfo): void {
  ...
  const hasOperationCol = table.columns.some(
    (c) =>
      c.name === "operation" ||
      c.name === "type" ||
      c.name === "state" ||
  ...
  const timestampCols = table.columns.filter(
    (c) => c.name.endsWith("At") && c.type === "timestamp"
  );
  ...
  if (hasOperationCol && timestampCols.length > 0) {
    ...
    const hasCompositeIndex = table.indexes.some((idx) => {
      ...
      const hasOp = colNames.some(
        (c) =>
          c.includes("operation") ||
          c.includes("status") ||
          c.includes("type") ||
          c.includes("state")
      );
    ...
    if (!hasCompositeIndex) {
      const operationCol = table.columns.find(
        (c) =>
      ...
      issues.push({
        ...
        message:
          "Table should have composite index for tenant + operation/status + timestamp queries",
        severity: "warning",
  ...
  if (hasTableNameCol && hasRowKeyCol) {
    const hasRowKeyIndex = table.indexes.some((idx) => {
      const colNames = idx.columns.map((c) => c.replace(/^t\./, "").toLowerCase());
      return (
        colNames.some((c) => c.includes("tenantid")) &&
