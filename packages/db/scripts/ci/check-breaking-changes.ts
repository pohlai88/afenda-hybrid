...
function getGitDiff(): string[] {
  try {
    ...
    const diff = execSync(
      `git diff --name-only origin/${baseBranch}...HEAD -- src/schema-platform/`,
      { encoding: "utf-8" }
    ).trim();
...
function analyzeChangedMigrations(): void {
  ...
  for (const migrationPath of changedMigrations) {
    ...
    const dropTableMatches = content.matchAll(
      /DROP TABLE\s+(?:IF EXISTS\s+)?["']?(\w+)\.(\w+)["']?/gi
    );
    for (const match of dropTableMatches) {
      changes.push({
        ...
        suggestion:
          "Ensure all data is migrated and dependent code is updated. Consider soft-delete instead.",
      });
    }

    // Check for DROP COLUMN
    const dropColumnMatches = content.matchAll(
      /ALTER TABLE\s+["']?(\w+)\.(\w+)["']?\s+DROP COLUMN\s+(?:IF EXISTS\s+)?["']?(\w+)["']?/gi
    );
    for (const match of dropColumnMatches) {
      changes.push({
        ...
        migration: migrationName,
        suggestion:
          "Ensure no code references this column. Consider deprecation period before removal.",
      });
    }

    // Check for type changes
    const typeChangeMatches = content.matchAll(
      /ALTER TABLE\s+["']?(\w+)\.(\w+)["']?\s+ALTER COLUMN\s+["']?(\w+)["']?\s+(?:SET DATA\s+)?TYPE\s+(\w+)/gi
    );
    for (const match of typeChangeMatches) {
      changes.push({
        ...
        migration: migrationName,
        suggestion:
          "Verify type change is backward compatible. Consider using USING clause for data conversion.",
      });
    }

    // Check for NOT NULL additions without default
    const notNullMatches = content.matchAll(
      /ALTER TABLE\s+["']?(\w+)\.(\w+)["']?\s+ALTER COLUMN\s+["']?(\w+)["']?\s+SET NOT NULL/gi
    );
    for (const match of notNullMatches) {
      ...
      if (!hasDefault) {
        changes.push({
          ...
          migration: migrationName,
          suggestion:
            "Add a default value or ensure all existing rows have values before applying NOT NULL.",
        });
      }
    }

    // Check for enum value removals
    const enumRemoveMatches = content.matchAll(
      /ALTER TYPE\s+["']?(\w+)["']?\s+DROP VALUE\s+["']?(\w+)["']?/gi
    );
    ...
    const fkDropMatches = content.matchAll(
      /ALTER TABLE\s+["']?(\w+)\.(\w+)["']?\s+DROP CONSTRAINT\s+["']?(\w+)["']?/gi
    );
    for (const match of fkDropMatches) {
      if (match[3].toLowerCase().includes("fk") || match[3].toLowerCase().includes("foreign")) {
        changes.push({
          ...
          migration: migrationName,
          suggestion:
            "Verify this doesn't break referential integrity. Document the reason for removal.",
        });
      }
    }
  }
    ...
    for (const [tableName, snapshotTable] of Object.entries(lastSnapshot.tables)) {
      ...
      if (!currentTable) {
        ...
        changes.push({
          ...
          suggestion:
            "Verify if table was renamed, moved to different schema, or intentionally removed.",
        });
        continue;
      }
...
function printSummary(): void {
  ...
  if (breaking.length > 0) {
    ...
    for (const change of breaking) {
      console.log(
        `❌ [${change.type}] ${change.schema}.${change.table}${change.column ? `.${change.column}` : ""}`
      );
  ...
  if (warnings.length > 0) {
    ...
    for (const change of warnings) {
      console.log(
        `⚠️ [${change.type}] ${change.schema}.${change.table}${change.column ? `.${change.column}` : ""}`
      );
      console.log(`   ${change.message}`);
      if (change.migration) {
        console.log(`   Migration: ${change.migration}`);
  ...
  if (infos.length > 0) {
    ...
    for (const change of infos) {
      console.log(
        `ℹ️ [${change.type}] ${change.schema}.${change.table}${change.column ? `.${change.column}` : ""}`
      );
      console.log(`   ${change.message}`);
      console.log(`   💡 ${change.suggestion}`);
      console.log();
