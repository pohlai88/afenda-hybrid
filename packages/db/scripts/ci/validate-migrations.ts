...
function loadCustomSqlRegistry(registryPath: string): CustomSqlRegistry {
  ...
  try {
    ...
    console.warn(
      `⚠️  Failed to load custom SQL registry: ${error instanceof Error ? error.message : String(error)}`
    );
    return {};
  }
}
...
function calculateSnapshotChecksum(snapshot: unknown): string {
  ...
  const normalized = JSON.stringify(
    snapshot,
    Object.keys(snapshot as Record<string, unknown>).sort()
  );
  return createHash("sha256").update(normalized).digest("hex");
}
    ...
    if (!hasDrizzlePatterns && drizzleSql.length > 100) {
      if (!isExcepted(exceptions, migration.name, "possible-hand-written")) {
        errors.push({
          ...
          severity: "error",
          suggestion:
            "Generate with 'drizzle-kit generate' or add an explicit exception in scripts/config/migration-exceptions.json",
        });
      }
    }
