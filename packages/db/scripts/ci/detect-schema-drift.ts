...
function getUncommittedSchemaChanges(schemaDir: string): string[] {
  try {
    ...
    const gitStatus = execSync(`git status --porcelain ${schemaDir}`, {
      encoding: "utf-8",
      cwd: process.cwd(),
    }).trim();
    ...
    return gitStatus
      ...
      .map((line) => {
        // Parse git status output: " M src/schema-platform/core/tenants.ts"
...
function generateDriftRecommendation(uncommittedChanges: string[]): string {
