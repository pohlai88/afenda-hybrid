...
export function parseTableFile(filePath: string): TableInfo | null {
  ...
  while ((match = columnRegex.exec(content)) !== null) {
    ...
    columns.push({
      ...
      hasDefault:
        colContext.includes(".default") ||
        colContext.includes(".defaultNow()") ||
        colContext.includes(".defaultRandom()"),
      isPrimaryKey: colContext.includes(".primaryKey()"),
      isGenerated:
        colContext.includes("generatedAlwaysAs") ||
        colContext.includes("generatedAlwaysAsIdentity"),
      line: lineNum,
    });
  }
...
export function analyzeSchema(schemaDir: string): SchemaInfo[] {
  ...
  for (const entry of entries) {
    ...
    const files = walkDir(
      schemaPath,
      (f) => !f.includes("_relations") && !f.includes("_schema") && !f.endsWith("index.ts")
    );
...
async function main() {
  const schemaDir = path.join(process.cwd(), "src/schema-platform");
...
if (
  ...
  process.argv[1]?.endsWith("schema-analyzer.ts")
) {
