 ...
 *
 * Run with: pnpm tsx scripts/prepare/auto-fix-schema.ts [--dry-run]
 */
...
function addZodSchemaExports(table: TableInfo): void {
  ...
  if (!table.hasZodSchemas.select) {
    schemaExports.push(
      `export const ${singularName}SelectSchema = createSelectSchema(${table.name});`
    );
  }
  if (!table.hasZodSchemas.insert) {
    schemaExports.push(
      `export const ${singularName}InsertSchema = createInsertSchema(${table.name});`
    );
  }
...
export type ${idTypeName} = z.infer<typeof ${idTypeName}>;`;
  ...
  fixes.push({
    ...
    action: () => {
      ...
      if (schemaMatch) {
        newContent = newContent.replace(schemaMatch[0], `${brandedIdCode}\n\n${schemaMatch[0]}`);
      } else {
        const typeMatch = newContent.match(/export type \w+ = typeof/);
        if (typeMatch) {
          newContent = newContent.replace(typeMatch[0], `${brandedIdCode}\n\n${typeMatch[0]}`);
        }
      }
...
function addRelationsFile(schema: SchemaInfo): void {
  ...
  for (const table of schema.tables) {
    const relativePath = path
      .relative(schema.path, table.file)
      .replace(".ts", "")
      .replace(/\\/g, "/");
