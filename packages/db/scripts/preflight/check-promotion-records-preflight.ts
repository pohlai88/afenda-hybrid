 ...
 *
 * @see docs/preflight/preflight-promotion-records-approval.sql
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (approvalCount > 0 || duplicateGroups > 0) {
      console.error(
        "Promotion records preflight failed. See docs/preflight/preflight-promotion-records-approval.sql"
      );
      process.exit(1);
    }
  } finally {
