 ...
 *
 * @see docs/preflight/preflight-grievance-records-resolution.sql
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (count > 0) {
      console.error(
        "Grievance resolution preflight failed. See docs/preflight/preflight-grievance-records-resolution.sql"
      );
      process.exit(1);
    }
  } finally {
