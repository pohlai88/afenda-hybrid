...
export const organizationStatusEnum = coreSchema.enum("organization_status", [
...
/**
 * Organizational hierarchy per tenant. Trigger `core.trg_organizations_same_tenant_parent` (CSQL-009) requires
 * `parentOrganizationId` to reference a row with the same `tenantId`.
 */
export const organizations = coreSchema.table(
  "organizations",
  {
    ...
    // status: organization_status — ACTIVE (default), INACTIVE, ARCHIVED
