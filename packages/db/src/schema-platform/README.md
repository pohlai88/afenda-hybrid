...
| **`index.ts`**  | —                         | **Drizzle Kit schema entry** (`packages/db/drizzle.config.ts` → `./src/schema-platform/index.ts`) |
...
| **`_queries` / `_services` / `db.ts`**               | Use **`@db/schema-platform`**, **`@db/schema-hrm/...`**, **`@db/db`** — same rule as other areas under `packages/db/src`.                                             |
