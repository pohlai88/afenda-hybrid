 ...
 * Run: pnpm test:db -- src/__tests__/social-insurance-profiles-zod.test.ts
 *
...
describe("social insurance profile Zod schemas", () => {
  describe("socialInsuranceProfileInsertSchema", () => {
    ...
    it("accepts statutorySchemeId path with valid rates", () => {
      expect(socialInsuranceProfileInsertSchema.safeParse(validInsertWithScheme).success).toBe(
        true
      );
    });
  });
