 ...
 * Run: pnpm test:db -- src/__tests__/background-checks-tenant-consistency.test.ts
 */
...
describe.skipIf(!process.env.DATABASE_URL)(
  "background checks create — tenant consistency (service)",
  () => {
    ...
    it("inserts when tenantId matches candidate", async () => {
      ...
      await db
        .delete(backgroundChecks)
        .where(eq(backgroundChecks.backgroundCheckId, row.backgroundCheckId));
    });
  }
);
