 ...
 * Run: pnpm test:db -- src/__tests__/applications-concurrency.test.ts
 */
...
describe.skipIf(!process.env.DATABASE_URL)(
  "applications partial unique (concurrent inserts)",
  () => {
    let tenantId: number;
    ...
    `;
      ...
      expect(
        isPgUnique ||
          matchesPgError(/23505|uq_applications_candidate_requisition|duplicate key|unique/i)(err)
      ).toBe(true);
    });
  }
);
