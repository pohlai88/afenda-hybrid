 ...
 * Run: pnpm test:db -- src/__tests__/statutory-scheme-rates-zod.test.ts
 */
...
describe("statutory scheme rate Zod schemas", () => {
  ...
  describe("statutorySchemeRateUpdateSchema", () => {
    ...
    it("rejects invalid rate on patch", () => {
      expectZodIssueAtPath(statutorySchemeRateUpdateSchema, { employeeRate: "2" }, "employeeRate");
    });
  });
});
...
describe("statutory scheme rates DB constraints", () => {
  ...
  afterAll(async () => {
    await db
      .delete(statutorySchemeRates)
      .where(eq(statutorySchemeRates.statutorySchemeId, schemeId));
