...
const talentContracts: Record<string, PgTableContract> = {
  ...
  competency_skills: {
    ...
    checkConstraintDefinitions: {
      ...
      chk_competency_skills_weight:
        /(?:"weight"|weight)\s+IS\s+NULL[\s\S]*(?:"weight"|weight)\s*>=\s*1/i,
    },
    foreignKeyDefinitions: {
      fk_competency_skills_tenant:
        /FOREIGN\s+KEY\s*\(\s*"tenantId"\s*\)[\s\S]*REFERENCES[\s\S]*tenants\s*\(\s*"tenantId"\s*\)[\s\S]*ON\s+DELETE\s+RESTRICT/i,
  ...
  disciplinary_actions: {
    ...
    checkConstraintDefinitions: {
      chk_disciplinary_actions_issue_after_incident: /"issueDate"\s*>=\s*"incidentDate"/i,
      chk_disciplinary_actions_expiry_after_issue:
    ...
    enumTypeLabels: {
      ...
      disciplinary_status: ["DRAFT", "ISSUED", "ACKNOWLEDGED", "APPEALED", "RESOLVED", "EXPIRED"],
    },
  },
  ...
  performance_review_goals: {
    ...
    triggerFunctions: {
      "talent.enforce_review_goal_final_score_vs_parent_status":
        /finalScore[\s\S]*reviewId|reviewId[\s\S]*finalScore/is,
    },
  ...
  performance_goals: {
    ...
    checkConstraintDefinitions: {
      ...
      chk_performance_goals_weight:
        /weight[\s\S]*IS\s+NULL[\s\S]*weight[\s\S]*>=\s*1[\s\S]*weight[\s\S]*<=\s*10/is,
      chk_performance_goals_completed_window:
  ...
  performance_reviews: {
    ...
    triggerFunctions: {
      "talent.enforce_review_status_vs_goal_final_scores":
        /performance_review_goals|finalScore|reviewId/is,
    },
    triggerFunctionIdentityArgs: {
      "talent.enforce_review_status_vs_goal_final_scores": "",
    },
  ...
  skills: {
    ...
    auditColumns: [...DEFAULT_TALENT_AUDIT_COLUMNS],
    enumTypeLabels: {
      skill_category: ["TECHNICAL", "SOFT", "LANGUAGE", "CERTIFICATION", "TOOL", "DOMAIN", "OTHER"],
  ...
  grievance_records: {
    ...
    checkConstraintDefinitions: {
      chk_grievance_records_resolution_consistency:
        /"resolvedBy"[\s\S]*"resolvedDate"[\s\S]*RESOLVED/is,
    },
    columnUdts: {
  ...
  promotion_records: {
    ...
    checkConstraintDefinitions: {
      ...
      chk_promotion_records_approval_consistency:
        /"approvedBy"[\s\S]*"approvedAt"[\s\S]*APPROVED|COMPLETED/is,
    },
    columnUdts: {
