 ...
 * Run: pnpm test:db -- src/__tests__/loan-records-zod.test.ts
 */
...
import {
  ...
  loanRecordUpdateSchema,
} from "../schema-hrm/payroll/operations/loanRecords";
  ...
  describe("loanRecordInsertSchema", () => {
    it("rejects non-positive principal", () => {
      expectZodIssueAtPath(
        loanRecordInsertSchema,
        { ...baseInsert, loanNumber: "LN-P0", principalAmount: "0" },
        "principalAmount"
      );
    });
    ...
    it("rejects negative principal", () => {
      expectZodIssueAtPath(
        loanRecordInsertSchema,
        { ...baseInsert, loanNumber: "LN-PN", principalAmount: "-100.00" },
        "principalAmount"
      );
    });
    ...
    it("rejects non-positive monthly deduction", () => {
      expectZodIssueAtPath(
        loanRecordInsertSchema,
        { ...baseInsert, loanNumber: "LN-M0", monthlyDeduction: "0" },
        "monthlyDeduction"
      );
    });
