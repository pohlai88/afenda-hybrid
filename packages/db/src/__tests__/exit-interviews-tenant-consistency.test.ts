 ...
 * Run: pnpm test:db -- src/__tests__/exit-interviews-tenant-consistency.test.ts
 */
...
describe.skipIf(!process.env.DATABASE_URL)(
  "exit_interviews create — linked checklist alignment (service)",
  () => {
    ...
    afterAll(async () => {
      await db
        .delete(exitInterviews)
        .where(eq(exitInterviews.linkedOffboardingChecklistId, checklistId));
      await db
        .delete(offboardingChecklists)
        .where(eq(offboardingChecklists.offboardingChecklistId, checklistId));
    ...
    it("inserts when tenantId and employeeId match linked checklist", async () => {
      ...
      await db
        .delete(exitInterviews)
        .where(eq(exitInterviews.exitInterviewId, row.exitInterviewId));
    });
  }
);
