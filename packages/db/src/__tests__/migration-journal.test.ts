...
const MIGRATIONS_ROOT = join(process.cwd(), "src/migrations");
...
describe("Database contract: migration journal", () => {
  it("drizzle.__drizzle_migrations matches src/migrations (names + sha256 of migration.sql)", async () => {
    ...
    for (const { name, hash } of local) {
      expect(
        dbByName.has(name),
        `Migration "${name}" exists locally but was not applied (missing journal row)`
      ).toBe(true);
      expect(
        dbByName.get(name),
    ...
    for (const name of dbByName.keys()) {
      expect(
        ...
        `Journal contains applied migration "${name}" but no src/migrations/<name>/migration.sql — orphan DB row`
      ).toBe(true);
    }
