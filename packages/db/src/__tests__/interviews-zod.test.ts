 ...
 * Run: pnpm test:db -- src/__tests__/interviews-zod.test.ts
 */
...
describe("interview Zod schemas", () => {
  describe("interviewInsertSchema", () => {
    ...
    it("rejects result when status omitted (defaults to SCHEDULED)", () => {
      expect(
        interviewInsertSchema.safeParse({ ...validInsertBase, result: "MAYBE" as const }).success
      ).toBe(false);
    });

    it("rejects non-positive tenantId", () => {
      expect(interviewInsertSchema.safeParse({ ...validInsertBase, tenantId: 0 }).success).toBe(
        false
      );
    });
  });
