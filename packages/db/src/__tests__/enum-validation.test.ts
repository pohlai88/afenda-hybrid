...
import { tenantStatusZodEnum, tenantSettingsSchema } from "../schema-platform/core/tenants";
import { regionTypeZodEnum, regionStatusZodEnum } from "../schema-platform/core/regions";
import {
...
describe("Enum Validation Tests", () => {
  ...
  describe("tenant settings validation", () => {
    ...
    it("rejects invalid locale format", () => {
      expect(() => tenantSettingsSchema.parse({ locale: "x" })).toThrow(); // too short
    });

    it("rejects unknown properties (strict mode)", () => {
      expect(() => tenantSettingsSchema.parse({ unknown: "value" })).toThrow();
    });
    ...
    it("validates features as boolean record", () => {
      ...
      expect(() => tenantSettingsSchema.parse({ features: { invalid: "not-boolean" } })).toThrow();
    });
  });
});
