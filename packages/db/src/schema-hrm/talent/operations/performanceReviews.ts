 ...
 * Preflight before adding constraints: docs/preflight/preflight-performance-reviews-lifecycle.sql
...
export const performanceReviews = talentSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_performance_reviews_period", sql`${t.reviewPeriodEnd} >= ${t.reviewPeriodStart}`),
    check(
