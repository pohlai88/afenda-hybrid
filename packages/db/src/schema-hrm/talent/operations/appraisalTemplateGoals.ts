...
export const appraisalTemplateGoals = talentSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    check("chk_appraisal_template_goals_weight", sql`${t.weight} >= 0 AND ${t.weight} <= 100`),
    check("chk_appraisal_template_goals_sequence", sql`${t.sequenceNumber} > 0`),
  ]
);

export const AppraisalTemplateGoalIdSchema = z
  .number()
  .int()
  .positive()
  .brand<"AppraisalTemplateGoalId">();
