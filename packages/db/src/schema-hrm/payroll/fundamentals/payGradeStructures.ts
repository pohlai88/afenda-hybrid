...
export const payGradeStructureStatusEnum = payrollSchema.enum("pay_grade_structure_status", [
  ...payGradeStructureStatuses,
]);
  ...
  if (mid !== undefined && mid !== null) {
    if (min !== undefined && min !== null && mid < min) {
      ctx.addIssue({
        code: "custom",
        message:
          "midSalary must be greater than or equal to minSalary when both are set in the patch",
        path: ["midSalary"],
      });
    }
