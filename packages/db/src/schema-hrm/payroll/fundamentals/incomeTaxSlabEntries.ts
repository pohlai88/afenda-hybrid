...
export const incomeTaxSlabEntries = payrollSchema.table(
  ...
  (t) => [
    ...
    check("chk_income_tax_slab_entries_rate", sql`${t.taxRate} >= 0 AND ${t.taxRate} <= 100`),
    check("chk_income_tax_slab_entries_from_amount", sql`${t.fromAmount} >= 0`),
    check("chk_income_tax_slab_entries_fixed_amount", sql`${t.fixedAmount} >= 0`),
  ]
);
...
export const incomeTaxSlabEntryInsertSchema = createInsertSchema(incomeTaxSlabEntries, {
  ...
  toAmount: z
    .string()
    .refine(isValidMoney15_2, "must be valid numeric(15,2) >= 0")
    .optional()
    .nullable(),
...
export const incomeTaxSlabEntryUpdateSchema = createUpdateSchema(incomeTaxSlabEntries, {
  ...
  taxRate: z
    .string()
    .refine(isValidTaxRate, "must be valid percentage 0-100 with max 2 decimals")
    .optional(),
  fixedAmount: z.string().refine(isValidMoney15_2, "must be valid numeric(15,2) >= 0").optional(),
