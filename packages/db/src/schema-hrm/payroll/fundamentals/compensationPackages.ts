...
export const compensationStatusEnum = payrollSchema.enum("compensation_status", [
...
const nonNegativeDecimalString = z.string().refine(
  (v) => {
...
export const compensationPackages = payrollSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_compensation_packages_salary", sql`${t.baseSalary} >= 0`),
    check(
...
export const CompensationPackageIdSchema = z
  .number()
  .int()
  .positive()
  .brand<"CompensationPackageId">();
