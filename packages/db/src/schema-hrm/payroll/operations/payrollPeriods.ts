...
export const payrollPeriods = payrollSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_payroll_periods_dates", sql`${t.periodEnd} >= ${t.periodStart}`),
    check("chk_payroll_periods_pay_date", sql`${t.payDate} >= ${t.periodEnd}`),
  ]
);
...
export const payrollPeriodUpdateSchema = createUpdateSchema(payrollPeriods, {
  ...
  .superRefine((data, ctx) => {
    ...
    refinePayrollPeriodDates(d.periodStart as Date, d.periodEnd as Date, d.payDate as Date, ctx);
  });
