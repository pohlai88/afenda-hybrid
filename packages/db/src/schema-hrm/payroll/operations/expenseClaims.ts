 ...
 */
export const expenseClaimStatuses = [
  "DRAFT",
  "SUBMITTED",
  "PENDING_APPROVAL",
  "APPROVED",
...
export const expenseClaimStatusEnum = payrollSchema.enum("expense_claim_status", [
...
function refineExpenseClaimInsertWorkflow(
  data: ExpenseClaimInsertWorkflow,
  ctx: z.RefinementCtx
): void {
  ...
  if (
    terminalNoRejection.includes(status) &&
...
function refineExpenseClaimUpdateWorkflow(
    ...
    if (status === "REJECTED") {
      if (!rrTouched || !hasRejection) {
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to REJECTED, include a non-empty rejectionReason in the same patch",
          path: ["rejectionReason"],
        });
      }
      if (
  ...
  if (
    Object.prototype.hasOwnProperty.call(data, "rejectionReason") &&
    rejectionReasonNonEmpty(data.rejectionReason)
  ) {
    ctx.addIssue({
      code: "custom",
...
export const expenseClaims = payrollSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_expense_claims_amount", sql`${t.amount} > 0`),
  ]
);
