...
const principalString = z.string().refine(isValidPrincipalString, {
  message:
    "principalAmount must be a positive decimal up to 9,999,999,999.99 with at most 2 decimal places (numeric 12,2)",
});

const totalAmountString = z.string().refine(isValidMoney12_2NonNegative, {
  message:
    "totalAmount must be a non-negative decimal up to 9,999,999,999.99 with at most 2 decimal places (numeric 12,2)",
});

const outstandingString = z.string().refine(isValidMoney12_2NonNegative, {
  message:
    "outstandingBalance must be a non-negative decimal up to 9,999,999,999.99 with at most 2 decimal places (numeric 12,2)",
});

const monthlyDeductionString = z.string().refine(isValidMonthlyDeductionString, {
  message:
    "monthlyDeduction must be a positive decimal up to 99,999,999.99 with at most 2 decimal places (numeric 10,2)",
});
...
const optionalInterestString = z
  ...
  .refine((s) => s === undefined || isValidInterestRateString(s), {
    message:
      "interestRate must be between 0 and 9.9999 with at most 4 decimal places (numeric 5,4)",
  });
...
const optionalTotalPaidString = z
  ...
  .refine((s) => s === undefined || isValidMoney12_2NonNegative(s), {
    message:
      "totalPaid must be a non-negative decimal up to 9,999,999,999.99 with at most 2 decimal places (numeric 12,2)",
  });
...
const optionalMoney12_2Patch = z
  ...
  .refine((s) => s === undefined || isValidMoney12_2NonNegative(s), {
    message:
      "Must be a non-negative decimal up to 9,999,999,999.99 with at most 2 decimal places (numeric 12,2)",
  });
  ...
  if (needsApproval) {
    if (!hasApprovedBy || !hasApprovedAt) {
      ctx.addIssue({
        code: "custom",
        message:
          "approvedBy and approvedAt are required for APPROVED, ACTIVE, COMPLETED, and DEFAULTED loans",
        path: ["approvedBy"],
      });
    }
  }
  ...
  if (statusTouched && status !== undefined) {
    ...
    if (needsApproval) {
      if (
        ...
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to APPROVED, ACTIVE, COMPLETED, or DEFAULTED, include approvedBy in the same patch",
          path: ["approvedBy"],
        });
      }
      if (!approvedAtTouched || approvedAt == null) {
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to APPROVED, ACTIVE, COMPLETED, or DEFAULTED, include approvedAt in the same patch",
          path: ["approvedAt"],
        });
      }
    }
    ...
    if (status === "ACTIVE" || status === "COMPLETED" || status === "DEFAULTED") {
      if (!disbursementTouched || disbursementDate == null) {
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to ACTIVE, COMPLETED, or DEFAULTED, include disbursementDate in the same patch",
          path: ["disbursementDate"],
        });
      }
    }
    ...
    if (status === "COMPLETED") {
      if (
        ...
        !isZeroMoney(outstandingBalance)
      ) {
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to COMPLETED, include outstandingBalance: 0 in the same patch",
          path: ["outstandingBalance"],
        });
      }
    }
...
export const loanRecords = payrollSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_loan_records_principal", sql`${t.principalAmount} > 0`),
    check("chk_loan_records_interest", sql`${t.interestRate} >= 0`),
    check("chk_loan_records_total", sql`${t.totalAmount} >= ${t.principalAmount}`),
    check("chk_loan_records_deduction", sql`${t.monthlyDeduction} > 0`),
    check("chk_loan_records_dates", sql`${t.endDate} > ${t.startDate}`),
    check("chk_loan_records_paid", sql`${t.totalPaid} >= 0 AND ${t.totalPaid} <= ${t.totalAmount}`),
    check("chk_loan_records_balance", sql`${t.outstandingBalance} >= 0`),
  ]
);
