...
export const settlementStatusEnum = payrollSchema.enum("settlement_status", [
  ...settlementStatuses,
]);
...
function settlementTotalsBalance(
...
function refineFinalSettlementInsertWorkflow(
  data: FinalSettlementInsertWorkflow,
  ctx: z.RefinementCtx
): void {
  ...
  if (status === "CANCELLED") {
    if (hasProcessedBy || hasProcessedAt || hasApprovedBy || hasApprovedAt || hasPaidAt) {
      ctx.addIssue({
        code: "custom",
        message:
          "processedBy, processedAt, approvedBy, approvedAt, and paidAt must be unset when status is CANCELLED",
        path: ["status"],
      });
    }
    return;
  ...
  if (needsApprovalMeta) {
    if (!hasApprovedBy || !hasApprovedAt) {
      ctx.addIssue({
        code: "custom",
        message:
          "approvedBy and approvedAt are required when status is APPROVED, PROCESSING, or PAID",
        path: ["approvedBy"],
      });
    }
    ...
    ctx.addIssue({
      code: "custom",
      message:
        "approvedBy and approvedAt are only allowed when status is APPROVED, PROCESSING, or PAID",
      path: ["approvedBy"],
    });
  }
...
function refineFinalSettlementUpdateWorkflow(
    ...
    if (needsApprovalMeta) {
      if (
        ...
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to APPROVED, PROCESSING, or PAID, include approvedBy in the same patch",
          path: ["approvedBy"],
        });
      }
      if (!approvedAtTouched || approvedAt == null) {
        ctx.addIssue({
          code: "custom",
          message:
            "When setting status to APPROVED, PROCESSING, or PAID, include approvedAt in the same patch",
          path: ["approvedAt"],
        });
      }
    } else {
  ...
  if ((abTouched && ab != null) || (aaTouched && aa != null)) {
    ctx.addIssue({
      code: "custom",
      message:
        "Include status (APPROVED, PROCESSING, or PAID) in the same patch when setting approval fields",
      path: ["approvedBy"],
    });
  }
}
...
export const finalSettlements = payrollSchema.table(
  ...
  (t) => [
    ...
    check("chk_final_settlements_dates", sql`${t.lastWorkingDay} <= ${t.terminationDate}`),
  ]
);
...
export const finalSettlementInsertSchema = finalSettlementInsertSchemaBase.superRefine(
  (data, ctx) => {
...
export const finalSettlementUpdateSchema = createUpdateSchema(finalSettlements, {
  ...
  .superRefine((data, ctx) => {
    ...
    if (lwT || termT) {
      ...
      if (
        !(lw instanceof Date) ||
        !(term instanceof Date) ||
        Number.isNaN(lw.getTime()) ||
        Number.isNaN(term.getTime())
      ) {
        return;
      }
