...
1. **Relations** — Only `_relations.ts` defines `defineRelations()`. It is merged with other domains in [`db.ts`](../../db.ts) (e.g. with `securityRelations`) for `db.query.*`.
...
| File                                            | Focus                                           |
| ----------------------------------------------- | ----------------------------------------------- |
| `__tests__/benefits-providers-zod.test.ts`      | Provider status, contract dates                 |
| `__tests__/benefit-plans-zod.test.ts`           | Plan enums, effective range, amounts            |
| `__tests__/benefit-enrollments-zod.test.ts`     | Enrollment status, coverage level, termination  |
| `__tests__/dependent-coverages-zod.test.ts`     | Coverage status vs level, termination           |
| `__tests__/claims-records-zod.test.ts`          | Claim status lifecycle, amounts, dates          |
| `__tests__/benefits-optional-relations.test.ts` | RQB optional `provider`, `currency`, `reviewer` |

Run all benefits-related DB tests:

```bash
pnpm test:db -- src/__tests__/benefits-
```
