...
export function refineRequiresCompletionDateIfCompleted<
  T extends { status?: unknown; completionDate?: unknown },
...
>(
  data: T,
  ctx: z.RefinementCtx,
  minAttendancePercent = LEARNING_MIN_ATTENDANCE_PERCENT_FOR_COMPLETED_DEFAULT
): void {
 ...
 */
export function refineAssessmentPassingVsMaxScore<
  T extends { maxScore?: unknown; passingScore?: unknown },
>(data: T, ctx: z.RefinementCtx): void {
 ...
 */
export function refineRequiresObjectivesIfActive<
  T extends { status?: unknown; objectives?: unknown },
>(data: T, ctx: z.RefinementCtx): void {
 ...
 */
export function refineRequiresPrerequisitesIfMandatory<
  T extends { isMandatory?: unknown; prerequisites?: unknown },
>(data: T, ctx: z.RefinementCtx): void {
 ...
 */
export function refineRequiresContentUrlForOnline<
  T extends { format?: unknown; contentUrl?: unknown },
>(data: T, ctx: z.RefinementCtx, options?: RefineCourseFormatOptions): void {
  const fmt = normEnumString(data.format) ?? options?.applyDefaultFormat;
  ...
  if (!isNonEmptyTrimmedText(data.contentUrl)) {
    ctx.addIssue({
      code: "custom",
      message:
        "contentUrl is required for ONLINE or SELF_PACED when validated together with format",
      path: ["contentUrl"],
    });
  }
}
