...
export const assessmentStatusEnum = learningSchema.enum("assessment_status", [
  ...assessmentStatuses,
]);
...
export const assessments = learningSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_assessments_max_score", sql`${t.maxScore} > 0`),
    check(
    ...
    check("chk_assessments_attempts", sql`${t.attempts} IS NULL OR ${t.attempts} >= 1`),
  ]
);
