...
| `_relations.ts` | `defineRelations()` graph for RQB (`learningRelations`), merged in [`db.ts`](../../db.ts)                                                                                                                                                                                                                     |
...
```bash
# From repository root (paths are relative to packages/db when using the workspace script):
pnpm test:db -- src/__tests__/learning-ld-enrollments-zod.test.ts
pnpm test:db -- src/__tests__/course-modules-zod.test.ts
pnpm test:db -- src/__tests__/courses-zod.test.ts
pnpm test:db -- src/__tests__/learning-path-courses-zod.test.ts
pnpm test:db -- src/__tests__/assessments-zod.test.ts
pnpm test:db -- src/__tests__/training-sessions-zod.test.ts
pnpm test:db -- src/__tests__/certification-awards-zod.test.ts
```
