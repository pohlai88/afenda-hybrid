 ...
 * For application-layer creates, use `createExitInterview` in `_services/recruitment/exitInterviewsService.ts` (validates linked checklist before insert; aligns with CSQL-015).
 *
...
export const exitInterviewFormatEnum = recruitmentSchema.enum("exit_interview_format", [
  ...exitInterviewFormats,
]);
...
export const exitInterviewStatusEnum = recruitmentSchema.enum("exit_interview_status", [
...
function refineExitInterviewCompletedTimingInsert(
...
export const exitInterviewInsertSchema = createInsertSchema(exitInterviews, {
  ...
  refineExitInterviewCompletedTimingInsert(row as Record<string, unknown>, ctx)
);
