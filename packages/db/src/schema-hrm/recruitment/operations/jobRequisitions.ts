 ...
 * for those links. Use `createJobRequisition` in `_services/recruitment/jobRequisitionsService.ts` for creates.
 *
...
export const requisitionTypeEnum = recruitmentSchema.enum("requisition_type", [
  ...requisitionTypes,
]);
...
export const requisitionStatusEnum = recruitmentSchema.enum("requisition_status", [
  ...requisitionStatuses,
]);
...
export const jobRequisitions = recruitmentSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_job_requisitions_headcount", sql`${t.headcount} >= 1`),
    check(
...
function refineJobRequisitionSalaryAndApproval(
...
export const jobRequisitionInsertSchema = createInsertSchema(jobRequisitions, {
  ...
  refineJobRequisitionSalaryAndApproval(row as Record<string, unknown>, ctx, "insert")
);
...
export const jobRequisitionUpdateSchema = createUpdateSchema(jobRequisitions, {
  ...
  minSalary: z
    .union([salaryAmountStringSchema, z.literal("")])
    .optional()
    .nullable(),
  maxSalary: z
    .union([salaryAmountStringSchema, z.literal("")])
