 ...
 * that equality. Use `createInterview` in `_services/recruitment/interviewsService.ts` for application-layer creates.
 *
...
export const interviewStatusEnum = recruitmentSchema.enum("interview_status", [
  ...interviewStatuses,
]);
...
export const interviewResultEnum = recruitmentSchema.enum("interview_result", [
  ...interviewResults,
]);
...
export const interviews = recruitmentSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    check("chk_interviews_duration", sql`${t.durationMinutes} IS NULL OR ${t.durationMinutes} > 0`),
    check(
...
function refineInterviewResultVsStatus(
...
export const interviewInsertSchema = createInsertSchema(interviews, {
  ...
  refineInterviewResultVsStatus(row as Record<string, unknown>, ctx, "insert")
);
...
export const interviewUpdateSchema = createUpdateSchema(interviews, {
  ...
  refineInterviewResultVsStatus(row as Record<string, unknown>, ctx, "update")
);
