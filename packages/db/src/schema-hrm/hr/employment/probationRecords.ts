...
import {
  ...
  dateStringSchema,
  nullableOptional,
  refineProbationActualEndOnOrAfterStart,
  refineProbationExtendedOnOrAfterOriginal,
  refineProbationOriginalEndOnOrAfterStart,
} from "../_zodShared";
...
export const probationOutcomeZodEnum = z.enum(probationOutcomes);
...
export const probationRecords = hrSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_probation_records_original_dates", sql`${t.originalEndDate} >= ${t.startDate}`),
    check(
...
export const probationRecordInsertSchema = createInsertSchema(probationRecords, {
  ...
  refineProbationOriginalEndOnOrAfterStart(data, ctx);
  refineProbationExtendedOnOrAfterOriginal(data, ctx);
  refineProbationActualEndOnOrAfterStart(data, ctx);
});
...
export const probationRecordUpdateSchema = createUpdateSchema(probationRecords, {
  ...
  extendedEndDate: dateNullableOptionalSchema,
  actualEndDate: dateNullableOptionalSchema,
  reviewNotes: nullableOptional(z.string().max(4000)),
  reviewedBy: nullableOptional(z.number().int()),
  reviewDate: dateNullableOptionalSchema,
}).superRefine((data, ctx) => {
  refineProbationOriginalEndOnOrAfterStart(data, ctx);
  refineProbationExtendedOnOrAfterOriginal(data, ctx);
  refineProbationActualEndOnOrAfterStart(data, ctx);
