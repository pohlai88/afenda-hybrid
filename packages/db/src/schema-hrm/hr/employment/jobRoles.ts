...
export const jobRoleStatusZodEnum = z.enum(jobRoleStatuses);
...
export const jobRoleUpdateSchema = createUpdateSchema(jobRoles, {
  ...
  jobFamilyId: nullableOptional(z.number().int()),
  responsibilities: nullableOptional(z.string().max(4000)),
  qualifications: nullableOptional(z.string().max(4000)),
});
