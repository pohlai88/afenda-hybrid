...
export const leaveRequestStatusEnum = hrSchema.enum("leave_request_status", [
...
export const leaveRequests = hrSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_leave_requests_date_range", sql`${t.endDate} >= ${t.startDate}`),
    check("chk_leave_requests_total_days", sql`${t.totalDays} > 0`),
    check(
