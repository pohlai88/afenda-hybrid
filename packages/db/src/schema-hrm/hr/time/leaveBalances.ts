...
import {
  nullableOptional,
  refineLeaveBalanceQuantityStringsNonNegative,
...
export const leaveBalances = hrSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("restrict")
      .onUpdate("cascade"),
    check("chk_leave_balances_entitled", sql`${t.entitled} >= 0`),
    check("chk_leave_balances_used", sql`${t.used} >= 0`),
    check("chk_leave_balances_pending", sql`${t.pending} >= 0`),
    check("chk_leave_balances_carried_over", sql`${t.carriedOver} >= 0`),
  ]
);
...
export const leaveBalanceInsertSchema = createInsertSchema(leaveBalances, {
  ...
  entitled: timesheetHoursStringSchema.optional(),
  used: timesheetHoursStringSchema.optional(),
  pending: timesheetHoursStringSchema.optional(),
  carriedOver: timesheetHoursStringSchema.optional(),
  adjustment: z.string().optional(),
}).superRefine((data, ctx) =>
  refineLeaveBalanceQuantityStringsNonNegative(data as Record<string, unknown>, ctx)
);

export const leaveBalanceUpdateSchema = createUpdateSchema(leaveBalances, {
  entitled: nullableOptional(timesheetHoursStringSchema),
  used: nullableOptional(timesheetHoursStringSchema),
  pending: nullableOptional(timesheetHoursStringSchema),
  carriedOver: nullableOptional(timesheetHoursStringSchema),
  adjustment: nullableOptional(z.string()),
}).superRefine((data, ctx) =>
  refineLeaveBalanceQuantityStringsNonNegative(data as Record<string, unknown>, ctx)
);
