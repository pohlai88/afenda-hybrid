...
export const emergencyRelationshipEnum = hrSchema.enum("emergency_relationship", [
  ...emergencyRelationships,
]);

export const emergencyRelationshipZodEnum = z.enum(emergencyRelationships);
...
export const emergencyContacts = hrSchema.table(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    check("chk_emergency_contacts_priority", sql`${t.priority} >= 1 AND ${t.priority} <= 10`),
  ]
);
...
export const emergencyContactUpdateSchema = createUpdateSchema(emergencyContacts, {
  ...
  alternatePhone: nullableOptional(z.string().max(30)),
