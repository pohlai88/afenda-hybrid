...
import { dateNullableOptionalSchema, dateStringSchema, nullableOptional } from "../_zodShared";
...
export const documentTypeZodEnum = z.enum(documentTypes);
...
export const documentStatusZodEnum = z.enum(documentStatuses);
...
export const personDocuments = hrSchema.table(
  ...
  (t) => [
    ...
    index("idx_person_documents_tenant_op_date").on(t.tenantId, t.status, t.uploadedAt),
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    check("chk_person_documents_file_size", sql`${t.fileSize} IS NULL OR ${t.fileSize} > 0`),
  ]
);
...
export const personDocumentUpdateSchema = createUpdateSchema(personDocuments, {
  ...
  fileSize: nullableOptional(z.number().int().positive()),
