...
| **Callers elsewhere under `packages/db/src`** | Prefer **`@db/_session`** for consistency with other cross-area imports.                             |
...
Session behavior is usually covered indirectly through integration tests that run against a real Postgres with audit triggers. If you add focused tests, keep them under [`__tests__/`](../__tests__/) and import via `@db/_session`.
