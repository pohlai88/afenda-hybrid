...
| **Cross-area** under `packages/db/src` | Use the **`@db/*`** alias: `@db/db`, `@db/schema-hrm/...`, etc.        |
...
Integration or contract tests live under [`__tests__/`](../__tests__/). Prefer importing the public barrel, e.g. `@db/_queries/talent`, so paths match production import style.
