...
export async function approveLeaveRequest(ctx: LeaveServiceContext, leaveRequestId: number) {
  await setSessionContext(ctx.tenantId, ctx.userId);
...
// Usage
if (!canApprove.allowed) {
  throw new ServiceError(`Cannot approve leave: ${canApprove.reason}`, "UNAUTHORIZED", {
