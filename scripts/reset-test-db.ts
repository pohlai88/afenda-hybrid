 ...
 * then applies Drizzle migrations from src/db/migrations.
 *
...
function assertSafeIdentifier(name: string, label: string): void {
  if (!/^[a-z_][a-z0-9_]*$/.test(name)) {
    throw new Error(
      `${label} must match /^[a-z_][a-z0-9_]*$/ for this script (got: ${name})`,
    );
  }
}
...
async function recreateDatabase(
  ...
  try {
    ...
    await admin.query(
      `CREATE DATABASE ${dbName} OWNER ${ownerRole} TEMPLATE template0`,
    );
  } finally {
    await admin.end();
  }
...
async function main(): Promise<void> {
  ...
  const allowed =
    dbName === "afenda_test" || process.env.AFENDA_FORCE_DB_RESET === "1";
  if (!allowed) {
    console.error(
