 ...
 *
 * @see docs/preflight-performance-reviews-lifecycle.sql
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (completedCount > 0 || acknowledgedCount > 0 || terminalOutcomesCount > 0) {
      console.error(
        "Performance reviews lifecycle preflight failed. Remediate using docs/preflight-performance-reviews-lifecycle.sql then re-run.",
      );
      process.exit(1);
    }
  } finally {
