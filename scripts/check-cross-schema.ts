    ...
    if (targetSchema !== schema.name && targetSchema !== "_shared") {
      ...
      if (tableDefStart > -1) {
        ...
        while ((refMatch = refRegex.exec(tableDef)) !== null) {
          ...
          if (content.includes(`import { ${refTable}`) && content.includes(`from "../${targetSchema}/`)) {
  ...
  for (const ref of crossSchemaRefs) {
    ...
    if (sourceTier < targetTier && ref.sourceSchema !== "audit") {
      issues.push({
        ...
        severity: "error",
        suggestion: "Lower tier schemas should not depend on higher tier schemas. Consider moving the referenced table to a lower tier or restructuring the relationship.",
      });
    }
  ...
  for (const node of graph.keys()) {
    ...
    if (hasCycle(node)) {
      ...
      issues.push({
        ...
        severity: "error",
        suggestion: "Break the circular dependency by moving shared types to a common schema or using a different relationship pattern",
      });
      break;
    }
  }
...
function main(): void {
  ...
  for (const [rule, ruleIssues] of Object.entries(byRule)) {
    ...
    for (const issue of ruleIssues) {
      ...
      console.log(`   ${issue.sourceSchema}.${issue.sourceTable} -> ${issue.targetSchema}.${issue.targetTable}`);
