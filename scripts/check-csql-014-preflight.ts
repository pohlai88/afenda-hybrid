 ...
 *
 * @see docs/preflight-csql-014-review-goal-final-score.sql
 */
...
async function main(): Promise<void> {
  if (process.env.SKIP_CSQL014_PREFLIGHT === "1") {
    console.log("check-csql-014-preflight: SKIP_CSQL014_PREFLIGHT=1 — skipping (no DB in this job).");
    process.exit(0);
  }
  ...
  try {
    ...
    if (violationCount > 0 || orphanCount > 0) {
      console.error(
        "CSQL-014 preflight failed. Remediate using docs/preflight-csql-014-review-goal-final-score.sql then re-run.",
      );
      process.exit(1);
    }
  } finally {
