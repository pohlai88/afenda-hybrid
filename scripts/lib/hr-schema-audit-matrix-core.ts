/**
 * Shared rules for `docs/hr-schema-audit-matrix.md`.
 * Used by `scripts/verify-hr-schema-audit-matrix.ts` and `src/db/__tests__/hr-schema-audit-matrix.test.ts`.
 */

/** Number of data rows (`| # | schema | table | ...`) in the matrix table body. */
export const EXPECTED_TABLE_ROWS = 115;

/** Third column where schema is `talent` — must appear exactly once each. Sync with docs/talent-schema-inventory.md. */
...
export function verifyHrSchemaAuditMatrix(text: string): AuditMatrixVerificationResult {
  ...
  if (rowMatches.length !== EXPECTED_TABLE_ROWS) {
    errors.push(
      `Expected ${EXPECTED_TABLE_ROWS} data rows, found ${rowMatches.length}. Update the matrix or EXPECTED_TABLE_ROWS in scripts/lib/hr-schema-audit-matrix-core.ts.`,
    );
  }
  ...
  if (recDupes.length > 0) {
    errors.push(
      `Recruitment section: duplicate table rows: ${[...new Set(recDupes)].join(", ")}`,
    );
  }
